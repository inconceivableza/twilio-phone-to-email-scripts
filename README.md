...
The wizard will:
1. Validate your Twilio credentials (API keys or auth tokens, per region)
2. Detect existing configuration and services
3. Recommend and configure regions per phone number, collect regional credentials
4. Configure per-number settings (email, SMS forwarding)
5. Set up email forwarding via Mailjet (optional)
6. Create a SIP domain and credentials (optional)
7. Write `.env` (non-secret config) and optionally `.secrets` (credentials)
8. Deploy functions to the required region(s) using regional credentials
...
```
node setup.js --account-sid ACxxx --api-key-sid SKxxx --api-key-secret xxx --api-key-region us1 --non-interactive --skip-sip
```
...
Available regions:
- **us1** — Virginia, USA (voice, SMS, functions, SIP, recording, transcription)
- **ie1** — Dublin, Ireland (voice, functions, SIP, recording, transcription)
- **au1** — Sydney, Australia (voice, SIP, recording — no functions or transcription)

Each region requires its own credentials (API key or auth token). The wizard auto-recommends regions based on phone number country codes.
...
├── .env                              ← Non-secret config (generated by setup)
├── .secrets                          ← Credentials (generated by setup, gitignored)
├── .env.example                      ← Template
├── data/
│   ├── regions.json                  ← Region definitions (features, edges)
│   └── country-regions.json          ← Country code → preferred region mapping
├── functions/
...
Twilio uses per-region credentials. Each region has its own auth token, and API keys are also region-specific. The setup wizard handles credential collection and validation, but you can also configure them manually.

### API keys (recommended)

Create a **standard** (not restricted) API key for each region you need. Restricted API keys do not support Serverless Functions APIs, which are needed for deployment.

1. Go to the Twilio Console for the target region:
   - **US1**: https://console.twilio.com/ → Account → API keys & tokens
   - **IE1**: https://console.ie1.twilio.com/ → Account → API keys & tokens
   - **AU1**: https://console.au1.twilio.com/ → Account → API keys & tokens
2. Click "Create API key" and choose **Standard**
3. Save the SID (`SKxxx...`) and Secret

### Credential storage

The setup wizard can save credentials to a `.secrets` file (gitignored, mode 0600) or you can set environment variables:

```bash
# .secrets file format (same as .env key=value):
ACCOUNT_SID=ACxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
API_KEY_SID_US1=SKxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
API_KEY_SECRET_US1=xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
API_KEY_SID_IE1=SKxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
API_KEY_SECRET_IE1=xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
MAILJET_API_KEY=xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
MAILJET_API_SECRET=xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
```

Alternatively, use auth tokens per region (not recommended for production):
```
AUTH_TOKEN_US1=xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
AUTH_TOKEN_IE1=xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
```
...
Edit your `.env` file (non-secret config) and `.secrets` file (credentials). See `.env.example` and the Credentials section above.

**Required (in `.secrets` or env vars):**
   - `ACCOUNT_SID`: Your Twilio Account SID
   - Per-region API key: `API_KEY_SID_{REGION}` + `API_KEY_SECRET_{REGION}` (recommended)
   - Or per-region auth token: `AUTH_TOKEN_{REGION}`

**Optional (in `.secrets` or env vars — leave blank to skip email):**
...
**Optional (regions, in `.env`):**
   - `VOICE_REGION`: Default voice processing region (`us1`, `ie1`, or `au1`, default: `us1`)
   - `SMS_REGION`: Default SMS processing region (`us1`, `ie1`, or `au1`, default: `us1`)
...
- **Multi-Region**: Deploy to us1 (Virginia), ie1 (Dublin), and/or au1 (Sydney) for lowest latency
